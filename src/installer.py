...
def check_env(keypath, network, FN):
    for tool in ["wget", "curl"]:
        if subprocess.call(["which", tool], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) != 0:
            print(f"\033[31;01m{tool}\033[00m has not been installed properly!\n")
            exit(1)
...
def set_binaries(FINDORAD_IMG, ROOT_DIR):
    if subprocess.run(["docker", "pull", FINDORAD_IMG], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode != 0:
        print("Failed to pull Docker image.")
        exit(1)

    if subprocess.run(["wget", "-T", "10", "https://github.com/FindoraNetwork/findora-wiki-docs/raw/main/.gitbook/assets/fn", "-O", "fn", "-q"]).returncode != 0:
...
def install_fn_app():
    # Install fn App
    subprocess.run(["wget", "https://github.com/FindoraNetwork/findora-wiki-docs/raw/main/.gitbook/assets/fn"], check=True)
...
def config_local_node(keypath, ROOT_DIR, USERNAME, SERV_URL, network, FINDORAD_IMG, CONTAINER_NAME, FN):
    ...
    # Tendermint config
    subprocess.run(["docker", "run", "--rm", "-v", f"{ROOT_DIR}/tendermint:/root/.tendermint", FINDORAD_IMG, "init", f"--{network}"], check=True)
...
def create_local_node(ROOT_DIR):
    ...
    subprocess.run([
        "docker", "run", "-d",
        "-v", f"{ROOT_DIR}/tendermint:/root/.tendermint",
        "-v", f"{ROOT_DIR}/findorad:/tmp/findora",
        "-p", "8669:8669", "-p", "8668:8668", "-p", "8667:8667", "-p", "8545:8545", "-p", "26657:26657",
        "-e", "EVM_CHAIN_ID=2152",
        "--name", CONTAINER_NAME,
        FINDORAD_IMG, "node",
